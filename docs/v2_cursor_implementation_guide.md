...
Progress lives in git-tracked [`.cursor/v2_implementation_loop.json`](../.cursor/v2_implementation_loop.json). **One user `/run-v2-implementation` per mode stretch.** If a turn ends before the macro block finishes, [`.cursor/hooks/v2-loop-auto-resume.sh`](../.cursor/hooks/v2-loop-auto-resume.sh) auto-submits the command again using the recorded `active_batch_mode` — you do not re-invoke manually. The hook does nothing when no batch is active. Any other user prompt clears `active_batch_mode` via [`.cursor/hooks/v2-loop-clear-on-other-prompt.sh`](../.cursor/hooks/v2-loop-clear-on-other-prompt.sh). Mid-batch false “batch complete” while `next_required_mode` still matches the current mode is a protocol violation.
